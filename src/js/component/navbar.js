					...
					<div className="btn-group">
						<button className="btn btnFavorite text-decoration-none fw-bold rounded-pill p-2 bg-gradient" disabled={favorites.length===0} type="button" data-bs-toggle="dropdown" aria-expanded="false">
							...
							favorites.map ((favorite, i)=>{ 
								return	(
									<li key={i} className="d-flex liFavorite dropdown-item"><Link to={linkSomePlace(favorite)} className="text-decoration-none liFavorite"><p className="card-title ">{favorite.name}</p></Link><button type="button" className=" fa-regular fa-trash-can border border-0 bg-transparent btnDeleteFav pb-2" aria-label="Delete" onClick={() => deleteFavorites(i)}></button></li>
									)
							})
							}
							</ul>
